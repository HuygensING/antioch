...
public class OptimisticAntiochClient {
  final AntiochClient delegate;

  // constructors

  public OptimisticAntiochClient(final URI antiochURI) {
    delegate = new AntiochClient(antiochURI);
  }

  public OptimisticAntiochClient(final String antiochURI) {
    this(URI.create(antiochURI));
  }

  public OptimisticAntiochClient(final URI antiochURI, SSLContext sslContext) {
    delegate = new AntiochClient(antiochURI, sslContext);
  }

  public OptimisticAntiochClient(final String antiochURI, SSLContext sslContext) {
    this(URI.create(antiochURI), sslContext);
  }
  ...
  public UUID addSearch(AntiochQuery query) {
  ...
  private <T> T unwrap(RestResult<T> restResult) {
    if (restResult.hasFailed()) {
      ...
      throw new AntiochException(message);
    }
