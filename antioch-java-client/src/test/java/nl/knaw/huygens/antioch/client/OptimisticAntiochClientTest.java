...
public class OptimisticAntiochClientTest extends AntiochTestWithTestServer {

  private static final String EVERYTHING_UPTO_AND_INCLUDING_THE_LAST_PERIOD_REGEX = ".*\\.";
  private static OptimisticAntiochClient client;

  @BeforeClass
  public static void startClient() {
    client = new OptimisticAntiochClient("http://localhost:2016/");
  }
  ...
  @Test
  public void testOptimisticAntiochClientHasDelegatedUnwrappedMethodForEachRelevantMethodInAntiochClient() {
    Class<AntiochClient> a = AntiochClient.class;

    String stubs = Arrays.stream(a.getMethods())//
        .filter(this::returnsRestResult)//
        .filter(this::hasNoDelegatedMethodInOptimisticAntiochClient)//
        .map(this::toDelegatedMethodStub)//
        .collect(joining("\n"));
    Log.info("Methods to add to OptimisticAntiochClient:\n{}", stubs);
  ...
  boolean hasNoDelegatedMethodInOptimisticAntiochClient(Method method) {
    Class<OptimisticAntiochClient> o = OptimisticAntiochClient.class;
    try {
