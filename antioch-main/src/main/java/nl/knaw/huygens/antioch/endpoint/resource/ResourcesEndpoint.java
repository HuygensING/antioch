 ...
 * #L%
 */
import static nl.knaw.huygens.antioch.api.EndpointPaths.ANNOTATIONS;
import static nl.knaw.huygens.antioch.api.EndpointPaths.RESOURCES;
import static nl.knaw.huygens.antioch.api.EndpointPaths.SUBRESOURCES;
import static nl.knaw.huygens.antioch.endpoint.resource.ResourceValidatorFactory.resourceNotFoundForId;
...
public class ResourcesEndpoint extends JSONEndpoint {
  ...
  @Inject
  public ResourcesEndpoint(AntiochService service, //
  ...
  public Response setResourceState(@PathParam("uuid") final UUIDParam uuidParam, @NotNull StatePrototype protoType) {
    // Log.trace("protoType=[{}]", protoType);
    AntiochResource resource = readExistingResource(uuidParam);
  ...
  public Response setResourceAtSpecificID(@PathParam("uuid") final UUIDParam uuid, //
    ...
    Optional<AntiochResource> existingResource = service.readResource(uuid.getValue());
  ...
  private Response created(AntiochResource resource) {
    return created(locationBuilder.locationOf(resource));
  }

  private Response ok(AntiochResource resource) {
    return ok(entityBuilder.build(resource));
  }

  private AntiochResource readExistingResource(UUIDParam id) {
    return service.readResource(id.getValue()).orElseThrow(resourceNotFoundForId(id));
  }

  private boolean isNotConfirmed(AntiochResource resource) {
    return !resource.getState().equals(AntiochState.CONFIRMED);
  }
