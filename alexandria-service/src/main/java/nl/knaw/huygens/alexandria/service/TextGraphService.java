...
public class TextGraphService {
  ...
  private Vertex getTextAnnotationVertex(TextAnnotation textAnnotation) {
    Object id = textAnnotation.getGraphId();
  ...
  private TextAnnotation toTextAnnotation(Vertex vertex) {
    ...
    if (textRangeAnnotations.hasNext()) {
      Iterator<VertexProperty<Object>> propertyIterator = textRangeAnnotations.next().properties(Storage.IDENTIFIER_PROPERTY);
      if (propertyIterator.hasNext()) {
        UUID uuid = UUID.fromString(propertyIterator.next().value().toString());
        textAnnotation.setUUID(uuid);
      }
    }
    textAnnotation.setGraphId(vertex.id());
    return textAnnotation;
  }
