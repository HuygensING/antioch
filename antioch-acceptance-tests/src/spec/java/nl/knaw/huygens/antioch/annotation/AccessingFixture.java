package nl.knaw.huygens.antioch.annotation;
...
public class AccessingFixture extends AnnotationsBase {
  ...
  public void resourceExistsWithAnnotationInState(String resIdStr, String annoState, String annoValue) {
    ...
    final AntiochAnnotationBody annoBody = service().createAnnotationBody(annoBodyId, "t", annoValue, aProvenance());
    final AntiochAnnotation annotation = service().annotate(theResource(resId), annoBody, aProvenance());
    ...
    case "tentative":
      ...
      Assert.assertEquals(AntiochState.TENTATIVE, annotation.getState());
      break;
    case "confirmed":
    ...
    case "deprecated":
      service().confirmAnnotation(annoId);
      final AntiochAnnotation revisedAnno = service().annotate(theResource(resId), annoBody, aProvenance());
